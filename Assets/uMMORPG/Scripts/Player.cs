...
public class Player : Entity {
                ...
                if (mp >= skill.manaCosts) {
                    ...
                    if (skill.category == "Attack") {
                        ...
                        if (target != null && target.hp > 0 &&
                            (target is Monster || (target is Player && canAttackPlayers))) {
                            // check range
                            // use collider point(s) to also work with big entities
                            if (Utils.ClosestDistance(collider, target.collider) > skill.castRange) {
                                // move to it first
                                agent.stoppingDistance = skill.castRange;
                                // use collider point(s) to also work with big entities
                                agent.destination = target.collider.ClosestPointOnBounds(tr.position);
                                state = "MOVING";
                                return;
                            } else {
        ...
        if (isServer) {
            ...
            // apply the skill after casting is finished
                ...
                if (HasEquipmentWeapon()) {
                    // attack?
                    if (skill.category == "Attack") {
                        // target alive and is a monster or a player?
                        if (target.hp > 0 &&
                            (target is Monster || (target is Player && canAttackPlayers))) {
                    ...
                    // heal?
                        ...
                        // only if alive (can't just heal a dead player)
                        if (castTarget.hp > 0) {
    ...
    public override void DealDamageAt(Entity entity, int n) {
        ...
        // did we attack a player?
        // (see murder code section comments to understand the system)
        } else if (entity is Player) {
            // was he innocent?
            if (!((Player)entity).IsOffender() && !((Player)entity).IsMurderer()) {
        ...
        if (CanStartTrade() &&
            ...
            if (go) {
                ...
                if (other.CanStartTrade() &&
                    Utils.ClosestDistance(collider, other.collider) <= talkRange) {
                        ...
                        if (entity is Monster) {
                            ...
                            if (entity.hp > 0) {
                                ...
                                // use collider point(s) to also work with big entities
                                    ...
                                    CmdNavigateTo(entity.collider.ClosestPointOnBounds(tr.position), skills.Count > 0 ? skills[0].castRange : 0f);
                            } else {
                                ...
                                // use collider point(s) to also work with big entities
                                    ...
                                    CmdNavigateTo(entity.collider.ClosestPointOnBounds(tr.position), lootRange);
                            }
                        } else if (entity is Player) {
                            ...
                            // use collider point(s) to also work with big entities
                                ...
                                CmdNavigateTo(entity.collider.ClosestPointOnBounds(tr.position), talkRange);
                        }
