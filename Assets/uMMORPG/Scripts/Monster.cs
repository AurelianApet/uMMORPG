    ...
    void UpdateIDLE() {
        if (isServer) {
            ...
            // event: aggro
                ...
                if (Vector3.Distance(start, target.collider.ClosestPointOnBounds(tr.position)) <= followDist) {
                    // in attack range?
                    // use collider point(s) to also work with big entities
                    if (Utils.ClosestDistance(collider, target.collider) <= atkRange) {
    ...
    void UpdateCASTING() {
        ...
        if (isServer) {
            ...
            // use collider point(s) to also work with big entities
                       ...
                       Vector3.Distance(start, target.collider.ClosestPointOnBounds(tr.position)) > followDist) {
                state = "IDLE";
                target = null;
        ...
        if (isServer) {
            ...
            if (Time.time >= deathTimeEnd) {
                ...
                if (respawn) {
                    ...
                    if (Time.time >= respawnTimeEnd) Respawn();
