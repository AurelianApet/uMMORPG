...
public class UIRefresh : MonoBehaviour {
    ...
    public Button npcTeleportButton;
    ...
    [Header("Shortcuts")]
    public Button shortcutInventory;
    public Button shortcutEquipment;
    public Button shortcutSkills;
    public Button shortcutCharacterInfo;
    public Button shortcutQuests;
    public Button shortcutQuit;
    ...
    void UpdateSkills(Player player) {
        ...
        for (int i = 0; i < skillsContent.childCount; ++i) {
            ...
            // get the skill (ignore status effects)
            if (i < player.skills.Count && !player.skills[i].category.StartsWith("Status")) {
    ...
    void UpdateNpcDialogue(Player player) {
        ...
        if (player.target != null && player.target is Npc &&
            ...
            // teleport button
            npcTeleportButton.gameObject.SetActive(npc.teleportTo != null);
            if (npc.teleportTo != null) npcTeleportButton.GetComponentInChildren<Text>().text = "Teleport to: " + npc.teleportTo.name;
            npcTeleportButton.onClick.SetListener(() => {
    ...
    void UpdateNpcQuest(Player player) {
        ...
        if (player.target != null && player.target is Npc &&
            ...
            if (idx != -1) {
                ...
                // accept
                npcQuestButtonAction.interactable = true;
    ...
    void UpdateShortcuts(Player player) {
        shortcutInventory.onClick.SetListener(() => {
            invPanel.SetActive(!invPanel.activeSelf);
        });

        shortcutEquipment.onClick.SetListener(() => {
            equipPanel.SetActive(!equipPanel.activeSelf);
        });

        shortcutSkills.onClick.SetListener(() => {
            skillsPanel.SetActive(!skillsPanel.activeSelf);
        });

        shortcutCharacterInfo.onClick.SetListener(() => {
            charInfoPanel.SetActive(!charInfoPanel.activeSelf);
        });

        shortcutQuests.onClick.SetListener(() => {
            questsPanel.SetActive(!questsPanel.activeSelf);
        });

        shortcutQuit.onClick.SetListener(() => {
            // use the NetworkManagerMMOUI quit function
            NetworkManager.singleton.GetComponent<NetworkManagerMMOUI>().OnButtonQuit();
        });
    }
